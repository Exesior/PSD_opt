"""
Minimal test to debug mass conservation issue.
Tests ONLY agglomeration first (no breakage/nucleation/compression).
"""
import sys
import os
...
print("=" * 80)
print("MASS CONSERVATION DEBUG TEST")
print("=" * 80)
...
# For porous: V_solid = V_dry × (1 - poro)
...
print(f"  Events:   {int(solver.real_agg_events)}")
...
if abs(mass_error) > 1e-6:
    ...
    # Check for particles with weird porosity
    if np.sum(valid_poro) > 0:
        print(f"\nPorous particles:")
        print(f"  Count:      {np.sum(valid_poro)}")
        print(f"  Mean poro:  {np.mean(poro_final[valid_poro]):.4f}")
        print(f"  Min/Max:    {np.min(poro_final[valid_poro]):.4f} / {np.max(poro_final[valid_poro]):.4f}")
        
        # Show first few porous particles
        porous_idx = np.where(valid_poro)[0][:5]
        print(f"\n  First {len(porous_idx)} porous particles:")
        for idx in porous_idx:
            print(f"    [{idx}] V_dry={v_dry_final[idx]:.6e}, poro={poro_final[idx]:.4f}, W={w_final[idx]:.1f}")
else:
    print("\n✓ MASS CONSERVED!")
