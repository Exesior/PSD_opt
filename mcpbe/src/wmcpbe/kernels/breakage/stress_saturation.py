...
class PowerLawRumpfBreakageKernel(BreakageKernel):
    ...
    def validate_params(self, params: dict) -> dict:
        ...
        for key in required:
            if key not in params:
                raise ValueError(
                    f"Missing required parameter '{key}' for stress_saturation breakage kernel."
                )
    ...
    def _compute_sigma(self, poro: float, saturation: float, x_s: float) -> float:
        ...
        Implements the 3-regime model:
        - S < 0.3: Dry regime (capillary bridges at contact points)
        - 0.3 ≤ S ≤ 0.8: Transition (linear interpolation)
        - S > 0.8: Wet regime (liquid film effects dominate)
        ...
        # Common factor: (1-poro)/poro × γ / x_s
        # Note: poro should already be validated (not NaN, in (0,1))
        poro_factor = (1.0 - poro) / poro
        base_factor = poro_factor * self.gamma / x_s
        
        # Three regimes
        if saturation < self.SAT_DRY_THRESHOLD:
            # Dry regime: σ = (1-poro)/poro × k × γ / x_s
            sigma = base_factor * self.k
            
        elif saturation > self.SAT_WET_THRESHOLD:
            # Wet regime: σ = 6×α × (1-poro)/poro × γ×cos(δ) / x_s × S
            sigma = 6.0 * self.alpha * base_factor * self.cos_delta * saturation
            
        else:
            # Transition regime: linear interpolation
            # σ(S=0.3) from dry formula
            sigma_dry = base_factor * self.k
            
            # σ(S=0.8) from wet formula (using S=0.8)
            sigma_wet = 6.0 * self.alpha * base_factor * self.cos_delta * self.SAT_WET_THRESHOLD
            
            # Linear interpolation: t = (S - 0.3) / 0.5
            t = (saturation - self.SAT_DRY_THRESHOLD) / self.SAT_TRANSITION_WIDTH
            sigma = (1.0 - t) * sigma_dry + t * sigma_wet
        
        return max(1e-30, float(sigma))  # Avoid division by zero
    ...
    def _compute_base_rate_powerlaw(self, v_particle: float) -> float:
        ...
        This matches the implementation in power_law.py exactly.
    ...
    def compute_rate(self, v_particle: float,
        ...
        try:
            ...
            if hasattr(solver, 'VERBOSE') and solver.VERBOSE:
                print(f"[stress_saturation] Warning: {e}, using base rate")
            return base_rate
