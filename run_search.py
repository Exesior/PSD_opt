import os
import sys

root = r"C:\Users\ericb\Documents\GitHub\PSD_opt"

# Suche nach allen Vorkommen von "nucleation" in .py Dateien
results = []

for dirpath, dirnames, filenames in os.walk(root):
    # Skip __pycache__ directories
    dirnames[:] = [d for d in dirnames if d != '__pycache__']
    for filename in filenames:
        if filename.endswith('.py'):
            filepath = os.path.join(dirpath, filename)
            try:
                with open(filepath, 'r', encoding='utf-8') as f:
                    content = f.read()
                    lines = content.split('\n')
                    for i, line in enumerate(lines, 1):
                        if 'nucleation' in line.lower():
                            results.append((filepath, i, line.rstrip()))
            except Exception as e:
                print(f"Error reading {filepath}: {e}", file=sys.stderr)

# Sortiere nach Datei und Zeile
results.sort(key=lambda x: (x[0], x[1]))

output_lines = []
for filepath, lineno, line in results:
    relpath = os.path.relpath(filepath, root)
    output_lines.append(f"{relpath}:{lineno}: {line[:200]}")

print('\n'.join(output_lines))

# Write to file for easier reading
with open(os.path.join(root, 'nucleation_results.txt'), 'w', encoding='utf-8') as f:
    f.write('\n'.join(output_lines))

print("\n\nResults also written to nucleation_results.txt")
